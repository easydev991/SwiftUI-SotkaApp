...
- **Загрузка инфопостов**: `loadAvailableInfoposts(currentDay:maxReadInfoPostDay:userGender:force:)`
  ...
  - Учитывает текущий день программы и максимальный доступный день с сервера.

- **Управление избранным**: `isFavorite(_:modelContext:)`, `changeFavorite(id:modelContext:)`
  - Проверяет и изменяет статус избранного для инфопостов.
  - Сохраняет изменения в SwiftData и обновляет локальное состояние.

- **Синхронизация прочитанных**: `syncReadPosts(modelContext:)`
...
- **Получение инфопоста по дню**: `getInfopost(forDay:)`
  - Возвращает инфопост для указанного дня.
...
- **Обработка ошибок**:
  ...
  - `ServiceError.infopostNotFound` - инфопост для указанного дня не найден.
