...
- [Сетевой слой удалён](#сетевой-слой-удалён)
...
- **Статистика прочитанных**: отслеживание прочитанных постов с локальным сохранением в SwiftData (сервер закрыт, синхронизации нет).
...
- Синхронизации с сервером нет: сервер `100.workout.su` закрыт (2026-08-01), приложение работает в офлайн-режиме.
- Устаревшие sync-поля (`unsyncedReadInfopostDays`) в модели `User` помечены `@available(*, deprecated)` и больше не заполняются.

### Сетевой слой удалён

Сервер `100.workout.su` закрыт, поэтому `InfopostsClient`/`SWClient` и сетевой sync-слой удалены полностью. Вся статистика прочитанных хранится только локально в SwiftData, сетевых операций в экране инфопостов нет.
...
- **Загрузка инфопостов**: `loadAvailableInfoposts(currentDay:maxReadInfoPostDay:userGender:force:)`
  ...
  - Учитывает текущий день программы и максимальный доступный день (`maxReadInfoPostDay`) через `InfopostAvailabilityManager`.
...
- **Отметка как прочитанного**: `markPostAsRead(day:modelContext:)`
  ...
  - Помечает пост как прочитанный локально (`addReadInfopostDay`) и сохраняет в SwiftData.

- **Проверка статуса прочитанного**: `isPostRead(_:modelContext:)`
  - Проверяет, прочитан ли инфопост, по локальному списку прочитанных дней (`readInfopostDays`).
...
- **Загрузка инфопоста "about"**: `loadAboutInfopost()`
  - Загружает инфопост "about" напрямую из файла через `Infopost(filename:language:)`.
...
- Unit-тесты для `InfopostsService` (23 теста) — проверка загрузки, фильтрации, избранного, отметки прочитанных.
- Unit-тесты для `InfopostParser` (12 тестов: `InfopostParserTests` + `InfopostParserYouTubeTests`) — проверка парсинга HTML файлов.
- Unit-тесты для `InfopostSectionDisplay` (8 тестов) — проверка модели отображения секций.
- Unit-тесты для `InfopostAvailabilityManager` (8 тестов) — проверка доступности по дням программы.
...
- Не добавлять сетевые операции в экран инфопостов (сервер закрыт, приложение в офлайн-режиме).
...
- Сохранять статистику прочитанных локально в SwiftData.
