...
- [x] Добавлены тесты продлений и миграции (включая legacy-store из релизов 4.0/4.1)
...
- [x] Добавить/обновить smoke-проверку preflight для UI-run:
...
- [x] Добавить check-list в план выполнения скриншотов: запуск на чистом симуляторе без ручных действий.

### Green: минимальная реализация

#### 11.1 Преднастройка симулятора для `make test_ui`

- [x] Вынести preflight-шаг в Makefile-пайплайн UI-тестов:
...
- [x] Применить тот же preflight-подход перед snapshot-прогоном (единый механизм для UI test + screenshots).
- [x] Зафиксировать, что snapshot-поток выполняется без ручных tap по системным alert-ам на новом симуляторе.

#### 11.3 Обновление fastlane

- [x] Обновить fastlane по рекомендациям инструмента: `bundle update fastlane`.
- [x] Проверить совместимость после обновления:
  - `bundle exec fastlane --version`;
  - smoke-run lane для скриншотов;
  - отсутствие новых регрессий в `make screenshots`.
- [x] Зафиксировать версию в `Gemfile.lock` и обновить связанное описание в документации проекта (если меняется команда/параметры).

### Refactor

- [x] Убрать дублирование preflight-логики между `make screenshots` и `make test_ui` (единая цель/скрипт).
- [x] Добавить краткий troubleshooting-блок в документацию (что делать при reset simulator, revoked permissions, stale simulator state).
- [x] Выполнить `make format` и повторный smoke-run:
  - `make ui_preflight_test_ui`;
  - `make ui_preflight_screenshots` (эквивалентный dry/smoke шаг lane).
...
Открытый хвост: отдельная задача по `VersionedSchema/SchemaMigrationPlan` (техдолг Этапа 2)
```
...
18. Безопасное открытие legacy SwiftData-хранилища проверено тестами: существующая БД предыдущей версии (включая релизные схемы 4.0/4.1) открывается без падений, данные сохраняются, `CalendarExtensionRecord` доступна.
