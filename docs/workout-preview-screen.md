...
- Создает модель `DayActivity` из данных ViewModel.
...
- Для дней 1-49 и 99-100: не показывается.
...
**Обработка нажатия "Начать тренировку"**: `startTraining()`
- Временно выводит в консоль TODO-сообщение.
- Полная реализация перехода к экрану выполнения тренировки будет добавлена в отдельном документе.
...
- Отображает счетчик кругов/подходов с возможностью изменения (отключен для типа `.turbo`).
- Показывает Picker для выбора времени отдыха (только для непройденных тренировок).
- Отображает поле для комментария (только для пройденных тренировок).
- Кнопки управления внизу экрана:
  - Для непройденных: "Начать тренировку" и "Сохранить как пройденную".
  - Для пройденных: "Комментарий" и "Сохранить".
...
- Отображает кнопки в зависимости от статуса тренировки (пройдена/не пройдена).
- Кнопка "Сохранить" активна только при наличии изменений (`hasChanges`).
...
- Показывать поле комментария только для пройденных тренировок.
...
- Реализовать переход к экрану выполнения тренировки при нажатии "Начать тренировку".
