...
- ✅ Extension для `WorkoutPreviewTraining` с методом `isTurboExercise` и тестами
- ✅ Метод `updateTrainings(_ newTrainings: [WorkoutPreviewTraining])` в `WorkoutPreviewViewModel` с тестами
- ✅ Метод `withCustomExercises` в `WorkoutProgramCreator` с тестами
- ✅ Метод `shouldShowEditButton()` в `WorkoutPreviewViewModel` с тестами
- ✅ Компонент `WorkoutExerciseEditorRowView` с двумя режимами (`.removable` и `.addable`)
- ✅ Экран `WorkoutExerciseEditorScreen` с тремя секциями (упражнения дня, стандартные, пользовательские)
- ✅ Кнопка редактирования в `WorkoutPreviewScreen` с условием показа только для `.cycles` и `.sets`
- ✅ Локализованные строки в `Localizable.xcstrings`
- ✅ Drag and drop для изменения порядка упражнений в первой секции (реализовано через `.onMove`)
- ✅ Открытие экрана редактора через `.sheet` вместо `.navigationDestination`
- ✅ Кнопки закрытия модальных окон (кнопка "xmark" в toolbar)

**Известные проблемы и TODO**:
- TODO: Преобразовать `shouldShowEditButton()` в вычисляемое свойство в `WorkoutPreviewViewModel`
- TODO: Вынести `standardExercises` в вычисляемое свойство в `ExerciseType` и написать тесты
- TODO: Не давать удалять единственное упражнение в первой секции редактора
- TODO: Убрать отображение `count` в `WorkoutExerciseEditorRowView` (отображать только иконку и название)
...
.toolbar {
    ToolbarItem(placement: .topBarLeading) {
        closeButton // Кнопка закрытия экрана
    }
    if viewModel.shouldShowEditButton() {
        ToolbarItem(placement: .topBarTrailing) {
            openEditorButton // Кнопка открытия редактора
        }
    }
}
...
1. **Секция 1: Упражнения текущего дня** (не турбо-упражнения)
   ...
   - **Поддержка drag and drop для изменения порядка** (только для этой секции, если получится реализовать)
...
2. **Секция 2: Стандартные упражнения**
   ...
   - **Drag and drop не нужен** для этой секции
...
- Кнопка закрытия (иконка "xmark") в навбаре слева для отмены изменений (изменения не сохраняются, локальный state-массив отбрасывается)
- Экран открывается через `.sheet` вместо `.navigationDestination` для модального отображения
...
- `onAppear` - инициализация `editableExercises` из `previewViewModel.trainings` (только не турбо-упражнения) с анимацией через `withAnimation`
- `removeExercise(_ exercise: WorkoutPreviewTraining)` - удаление упражнения из локального массива `editableExercises` с анимацией (TODO: не давать удалять единственное упражнение)
- `addStandardExercise(_ exerciseType: ExerciseType)` - добавление стандартного упражнения в локальный массив `editableExercises` с `count = 5` и анимацией
- `addCustomExercise(_ customExercise: CustomExercise)` - добавление пользовательского упражнения в локальный массив `editableExercises` с `count = 5` и анимацией
...
- `exerciseImage(for training: WorkoutPreviewTraining) -> Image` - получение иконки упражнения
- `exerciseTitle(for training: WorkoutPreviewTraining) -> String` - получение названия упражнения
- `standardExercises: [ExerciseType]` - список стандартных упражнений (TODO: вынести в `ExerciseType` как computed property)
...
- Параметры: `image`, `title`, `count`, `mode` (`.removable` или `.addable`), `onAction` (для удаления/добавления)
- Enum `WorkoutExerciseEditorRowMode` с computed property `systemImageName` для иконок кнопок
- Для первой секции: кнопка удаления слева (иконка "minus" с `.symbolVariant(.circle.fill)`, роль `.destructive`) + `ActivityRowView` + drag handle справа
- Для второй и третьей секций: кнопка добавления слева (иконка "plus" с `.symbolVariant(.circle.fill)`, цвет `.accentColor`) + `ActivityRowView`
- TODO: Убрать отображение `count` в редакторе (отображать только иконку и название)

**Drag and Drop**:
- Использование нативного `.onMove(perform:)` модификатора для `List` в SwiftUI
- **Применяется ТОЛЬКО к первой секции** (упражнения дня) - если получится реализовать
- **Для второй и третьей секций drag and drop не нужен** и не должен быть реализован
...
- Первая секция: `ForEach(editableExercises)` с `.onMove` для drag and drop (если получится реализовать)
- Вторая секция: `ForEach(standardExercises)` - стандартные упражнения (без drag and drop)
- Третья секция: `ForEach(customExercises)` - пользовательские упражнения (если есть, без drag and drop)
...
- Тест drag and drop в первой секции (если получится реализовать)
- Тест отсутствия drag and drop во второй и третьей секциях
- Тест добавления/удаления упражнений
- Тест сохранения изменений

##### 7. Локализация ✅

**Реализованные строки для Localizable.xcstrings**:
- ✅ "WorkoutExerciseEditor.Title" - "Редактор тренировки" (en: "Workout editor")
- ✅ "WorkoutExerciseEditor.Done" - "Готово" (en: "Done")
- ✅ "WorkoutExerciseEditor.DayExercises" - "Упражнения дня" (en: "Day exercises")
- ✅ "WorkoutExerciseEditor.StandardExercises" - "Стандартные упражнения" (en: "Standard exercises")
- ✅ "WorkoutExerciseEditor.CustomExercises" - "Пользовательские упражнения" (en: "Custom exercises")

**Использование**: Строки используются через extension для `LocalizedStringKey` (автоматически генерируются из `Localizable.xcstrings`):
- `.workoutExerciseEditorTitle` для заголовка экрана
- `.workoutExerciseEditorDone` для кнопки "Готово"
- `.workoutExerciseEditorDayExercises` для заголовка первой секции
- `.workoutExerciseEditorStandardExercises` для заголовка второй секции
- `.workoutExerciseEditorCustomExercises` для заголовка третьей секции
...
1. **Этап 1: Подготовка и тесты** ✅
   ...
   - ✅ Добавить метод `shouldShowEditButton()` в `WorkoutPreviewViewModel` с тестами
...
2. **Этап 2: UI компоненты** ✅
   ...
   - ✅ Создать enum `WorkoutExerciseEditorRowMode` с computed property `systemImageName`
   ...
   - ✅ Реализовать нативный `.onMove` для drag and drop **только в первой секции**
   - ✅ Реализовать методы добавления/удаления/перемещения упражнений в локальном массиве с анимацией
   ...
   - ✅ Добавить кнопку закрытия (иконка "xmark") в toolbar экрана редактора
   - ✅ Запустить `make format`
...
3. **Этап 3: Интеграция** ✅
   ...
   - ✅ Добавить `.sheet(isPresented: $showEditorScreen)` для модального отображения экрана редактора
   - ✅ Передать `viewModel` через `.environment(viewModel)` в экран редактора
   - ✅ Добавить кнопку закрытия (иконка "xmark") в toolbar экрана превью
   - ✅ Запустить `make format`

4. **Этап 4: Локализация** ✅
   - ✅ Добавить локализованные строки в `Localizable.xcstrings`
   - ✅ Использовать extension для `LocalizedStringKey` (автоматически генерируются)
   - ✅ Запустить все тесты
...
- **Модальное отображение**: Экран редактора открывается через `.sheet` для модального отображения
- **Анимации**: Все изменения в списке упражнений анимированы через `withAnimation`

##### 11. План доработок (TODO)

**Приоритет 1: Рефакторинг ViewModel** ✅
- ✅ Преобразован `shouldShowEditButton()` в computed property `shouldShowEditButton: Bool`
- ✅ Обновлены тесты и использование в `WorkoutPreviewScreen`

**Приоритет 2: Рефакторинг ExerciseType** ✅
- ✅ Вынесен `standardExercises` в `ExerciseType.standardExercises` с тестами
- ✅ Обновлен `WorkoutExerciseEditorScreen` для использования централизованного списка

**Приоритет 3: Валидация удаления упражнений** ✅
- ✅ Добавлена проверка `canRemoveExercise` и валидация в `removeExercise`
- ✅ Применен модификатор `.disabled` для предотвращения удаления последнего упражнения

**Приоритет 4: Улучшение UI компонента** ✅
- ✅ Убран параметр `count` из `WorkoutExerciseEditorRowView`
- ✅ Обновлены все места использования компонента

**Приоритет 5: Исправление парсинга ответа сервера при синхронизации тренировок** ✅
- **Проблема**: При синхронизации тренировки с измененными упражнениями возникала ошибка парсинга (код 4864) из-за того, что сервер возвращает числовые поля как строки
- **Решение**: Добавлены кастомные декодеры для `DayResponse` и `DayResponse.Training` с функцией `decodeIntOrString`, аналогично `ProgressResponse`. Теперь модели корректно обрабатывают числовые поля, приходящие как строки или числа
- **Реализовано**:
  - Кастомный `init(from decoder: Decoder)` для `DayResponse` с обработкой всех числовых полей (`id`, `activityType`, `count`, `plannedCount`, `executeType`, `trainType`, `duration`)
  - Кастомный `init(from decoder: Decoder)` для `DayResponse.Training` с обработкой числовых полей (`typeId`, `count`, `sortOrder`)
  - Написаны unit-тесты для всех сценариев декодирования (9 тестов)
  - Ошибка парсинга больше не возникает при синхронизации тренировок

**Анализ избыточного кода после исправления проблемы декодирования** ✅:
- ✅ **Удалено**: Временные DEBUG логи в `DailyActivitiesService.swift`, которые были добавлены для отладки проблемы с декодированием
  - Удалены логи `📤 [DEBUG] Параметры формы для дня X` и детальное логирование всех параметров формы
  - Удалены логи `📤 [DEBUG] Тренировка[X]: ...` с детальной информацией о sortOrder, count, typeId, customTypeId
  - Удален устаревший комментарий "Локальные отладочные логи перед выполнением задач"
  - Оставлены только основные информационные логи о синхронизации (отправка активности, параметры запроса)
- ✅ **Проверено**: Логика сортировки тренировок по `sortOrder` в `DayRequest.formParameters` - необходима и корректна (сортировка перед отправкой на сервер)
- ✅ **Проверено**: Метод `updateTrainings` в `WorkoutPreviewViewModel` - необходим для пересчета `sortOrder` при изменении списка упражнений в редакторе
- ✅ **Проверено**: Все остальные использования `sortOrder` в коде - необходимы для корректной работы редактора тренировок и синхронизации с сервером