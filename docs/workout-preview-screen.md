...
- При создании **новой** тренировки данные подставляются из последней пройденной тренировки через `WorkoutProgramCreator.withData(from:)` (см. раздел WorkoutProgramCreator): ищется последняя пройденная **не турбо**-тренировка (`DailyActivitiesService.getLastPassedNonTurboWorkoutActivity`), из неё берутся `plannedCount`, `executionType` и повторы для каждого упражнения; турбо-тренировки из поиска исключаются, сортировка — по дню и `modifyDate`. Если предыдущая не найдена — используются дефолтные значения по дню.
...
- Для одинаковых стандартных упражнений (например, двух строк с приседаниями) сохраняет разные пользовательские значения по порядку их появления в списке, а не только по `typeId`.
- Упражнения, добавленные в редакторе (сверх программы дня), сохраняются при переключении способа выполнения и идут после программных с обновлённым `sortOrder`.
...
- Подстановка данных из предыдущей пройденной тренировки через `withData(from: DayActivity)`: из переданной активности (должна быть не турбо) подставляются `plannedCount` (приоритет: `count` → `plannedCount`), `executionType` и повторы для каждого упражнения; сопоставление упражнений — по `typeId`/`customTypeId` и порядку появления в списке (для дублей одного типа сохраняются разные значения). Если предыдущая тренировка не передана, используются дефолты по дню.
...
- Корректно сопоставлять повторяющиеся упражнения одного типа при смене типа выполнения, чтобы не смешивать их значения между собой.
- При переключении способа выполнения сохранять добавленные в редакторе упражнения (не только программные).
...
- При создании новой тренировки подставлять данные из последней пройденной не турбо-тренировки через `withData(from:)`; турбо-тренировки из поиска исключать; при отсутствии предыдущей — использовать дефолты по дню.
