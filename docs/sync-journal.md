...
- [Модель данных: `SyncJournalEntry`](#модель-данных-syncjournalentry)
...
- `details: SyncResultDetails?` — детальная информация о результатах синхронизации (статистика по прогрессу/упражнениям/активностям, ошибки). Хранится как `Data` в приватном поле `detailsData` и сериализуется/десериализуется через JSON.
- `user: User?` — связь с пользователем через `@Relationship`.
- `duration: TimeInterval?` — вычисляемое свойство для длительности синхронизации (разница между `endDate` и `startDate`, nil если `endDate == nil`).
