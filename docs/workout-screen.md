...
- Для `.sets`: создает разминку, подходы для каждого упражнения:
  - Для турбо-дней с подходами: использует `WorkoutProgramCreator.isTurboWithSets()` для определения количества подходов (1 подход на упражнение).
  - Для остальных: по количеству `plannedCount`.
...
- Если `interrupt == true`: возвращает результат даже для незавершенной тренировки, считая только завершенные упражнения.
- Подсчитывает количество выполненных упражнений (кругов/подходов):
  - Для обычного завершения: общее количество этапов упражнений.
  - Для прерванной тренировки: только завершенные этапы упражнений.
...
- Логи: получение результата тренировки с указанием количества и длительности.
