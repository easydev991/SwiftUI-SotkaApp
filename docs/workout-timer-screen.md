...
- **Досрочное завершение**: возможность завершить отдых досрочно через кнопку "Завершить отдых".
...
- `"TimerScreen.FinishButton"` — "Завершить отдых" (кнопка досрочного завершения).
