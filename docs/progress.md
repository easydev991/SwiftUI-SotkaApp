...
- **ProgressScreen** — основной экран прогресса с статистикой и сеткой.
...
- **ProgressGridView** — сетка для отображения данных прогресса для контрольных точек.
...
- `GET v3/100/progress` — получение списка прогресса пользователя.
- `GET v3/100/progress/{day}` — получение прогресса для конкретного дня.
- `POST v3/100/progress` — создание нового прогресса.
- `POST v3/100/progress/{day}` — обновление существующего прогресса (используется для создания и обновления).
- `DELETE v3/100/progress/{day}` — удаление прогресса для конкретного дня.
- `DELETE v3/100/progress/{day}/photo/{type}` — удаление фотографии определенного типа.
...
- **Двунаправленная синхронизация**: `syncProgress(context:)`
...
- **Экран прогресса** (`ProgressScreen`):
  ...
  - Навигация к редактированию метрик или фотографий при нажатии на соответствующие элементы.
  - Фильтрует элементы с `shouldDelete = true` из отображения.

- **Сетка прогресса** (`ProgressGridView`):
  - Отображает данные в виде таблицы с заголовками для каждого дня.
  - Показывает метрики (подтягивания, отжимания, приседания, вес) с иконками.
...
- **Редактирование прогресса** (`EditProgressScreen`):
  ...
  - В режиме метрик: ввод данных упражнений и веса с валидацией и навигацией между полями.
  - В режиме фотографий: выбор фотографий из камеры или библиотеки с обработкой и удалением.
  - Кнопка сохранения доступна только при наличии изменений и валидных данных.
  - Кнопка удаления доступна только если прогресс содержит данные.
...
- **Главный экран** (`HomeFillProgressSectionView`):
  ...
  - Переход к экрану прогресса через `NavigationLink`.
