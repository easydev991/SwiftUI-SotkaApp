...
- ✅ Платформо-специфичные компоненты: `TextFieldLink` обернут в `#if os(watchOS)` для корректной компиляции
- ✅ UIKit API исключены из компиляции для watchOS: методы с `UIImpactFeedbackGenerator`, `UIViewControllerRepresentable` и др. обернуты в `#if !os(watchOS)` или помечены `@available(watchOS, unavailable)`
...
- [x] ✅ Платформо-специфичные компоненты: использование `TextFieldLink` (watchOS-only) для ввода текста на часах
...
- [x] ✅ Haptic feedback: `UIImpactFeedbackGenerator` недоступен на watchOS, метод `hapticFeedback` в `SWDesignSystem` исключен из компиляции для watchOS через `#if !os(watchOS)`. Для watchOS можно использовать `WKHapticType` при необходимости
...
**Проблема:**
После рефакторинга архитектуры WatchConnectivity некоторые unit-тесты основного приложения падают при выполнении `make test`. Тесты на часах (`make test_watch`) проходят успешно.

**Анализ падающих тестов:**

1. **StatusManagerIntegrationTests.swift:75** - тест "Повторный запуск с синхронизацией: getStatus → даты совпадают → syncJournalAndProgress"
   - **Ошибка:** Счетчики вызовов клиентов (`mockProgressClient.getProgressCallCount`, `mockExerciseClient.getCustomExercisesCallCount`, `mockDaysClient.getDaysCallCount`) не увеличиваются после вызова `getStatus()`
   - **Ожидание:** После `getStatus()` должен вызываться `syncJournalAndProgress()`, который вызывает сервисы, которые в свою очередь вызывают клиенты
   - **Причина:** Возможно, `syncJournalAndProgress()` не вызывается из-за условия `appDate.isTheSameDayIgnoringTime(siteDate)`, или сервисы не вызывают клиенты из-за отсутствия данных/пользователя

2. **StatusManagerGetStatusTests.swift:231** - тест "Вызывает syncJournalAndProgress, если даты совпадают"
   - **Ошибка:** Аналогичная проблема - счетчики вызовов клиентов не увеличиваются
   - **Причина:** Аналогично предыдущему тесту

3. **ProgressSyncServiceTests** - один тест падает
   - **Требуется детальный анализ** конкретного падающего теста

4. **StatusManagerTests** - 63 ошибки
   - **Требуется детальный анализ** всех падающих тестов

**План исправления:**

1. **Анализ логики вызова `syncJournalAndProgress()`:**
   - Проверить условие `appDate.isTheSameDayIgnoringTime(siteDate)` в тестах
   - Убедиться, что `startDate` устанавливается корректно после `startNewRun()`
   - Проверить, что `statusClient.current()` возвращает правильную дату

2. **Анализ вызовов клиентов в сервисах:**
   - `ProgressSyncService.syncProgress()` вызывает `client.getProgress()` в `downloadServerProgress()` только если есть пользователь
   - `DailyActivitiesService.syncDailyActivities()` вызывает `client.getDays()` в `downloadServerActivities()` только если есть пользователь
   - `CustomExercisesService.syncCustomExercises()` должен вызывать клиент аналогично
   - Убедиться, что пользователь создается в правильном `ModelContext` и доступен сервисам

3. **Исправление тестов:**
   - Убедиться, что пользователь создается в том же `ModelContainer`, который используется в `StatusManager`
   - Проверить, что `ModelContainer` передается в `StatusManager` через `MockStatusManager.create()`
   - Убедиться, что даты совпадают после `startNewRun()` и `getStatus()`
   - Возможно, нужно использовать `>=` вместо `>` для проверки счетчиков, если синхронизация может вызываться несколько раз

4. **Исправление логики (если необходимо):**
   - Если проблема в логике, а не в тестах, исправить условия вызова `syncJournalAndProgress()`
   - Убедиться, что сервисы всегда вызывают клиенты для загрузки данных с сервера, даже если нет локальных изменений

5. **Запуск и проверка:**
   - Запустить `make test` с `required_permissions: ['all']`
   - Убедиться, что все тесты проходят
   - Проверить, что тесты на часах (`make test_watch`) продолжают проходить

**Приоритет:** Высокий - тесты должны проходить для обеспечения качества кода
