...
#### 9.2 Неправильная логика создания подходов для турбо-дней с подходами на часах ⚠️ **АКТИВНЫЙ БАГ**
...
- Тест: Для дня 93 с типом выполнения `.turbo` должно создаваться 5 подходов (по одному на каждое упражнение)
  - Проверка: `stepStates.filter { if case .exercise(.sets, _) = $0.step { return true }; return false }.count == 5`
  - Проверка: Все подходы должны иметь номер 1 (локально для каждого упражнения)
- Тест: Для дня 95 с типом выполнения `.turbo` должно создаваться 5 подходов (по одному на каждое упражнение)
  - Проверка: `stepStates.filter { if case .exercise(.sets, _) = $0.step { return true }; return false }.count == 5`
- Тест: Для дня 98 с типом выполнения `.turbo` должно создаваться 3 подхода (по одному на каждое упражнение)
  - Проверка: `stepStates.filter { if case .exercise(.sets, _) = $0.step { return true }; return false }.count == 3`
- Тест: Для обычных дней с типом выполнения `.sets` и `plannedCount = 6` должно создаваться правильное количество подходов
  - Проверка: Для 4 упражнений должно быть 4 × 6 = 24 подхода
  - Проверка: Номера подходов должны быть от 1 до 6 для каждого упражнения

**Шаг 2: Написать тесты для `WorkoutViewModel.getExerciseSteps()` для турбо-дней с подходами**
- Тест: Для дня 93 метод `getExerciseSteps()` должен возвращать 1 подход для каждого упражнения
  - Проверка: Для каждого из 5 упражнений `getExerciseSteps(for: trainingId).count == 1`
- Тест: Для дня 95 метод `getExerciseSteps()` должен возвращать 1 подход для каждого упражнения
  - Проверка: Для каждого из 5 упражнений `getExerciseSteps(for: trainingId).count == 1`
- Тест: Для дня 98 метод `getExerciseSteps()` должен возвращать 1 подход для каждого упражнения
  - Проверка: Для каждого из 3 упражнений `getExerciseSteps(for: trainingId).count == 1`

**Шаг 3: Написать тесты для метода определения текущего упражнения (новый метод в `WorkoutViewModel`)**
- Тест: Для дня 93 с типом выполнения `.turbo` и глобальным номером подхода 1 должен возвращаться индекс упражнения 0
  - Проверка: Для турбо-дней с подходами: `exerciseIndex = number - 1`
- Тест: Для дня 93 с типом выполнения `.turbo` и глобальным номером подхода 5 должен возвращаться индекс упражнения 4
  - Проверка: Для турбо-дней с подходами: `exerciseIndex = number - 1`
- Тест: Для обычного дня (не турбо) с типом выполнения `.sets`, `plannedCount = 6` и глобальным номером подхода 1 должен возвращаться индекс упражнения 0
  - Проверка: Для обычных дней: `exerciseIndex = (number - 1) / plannedCount`
- Тест: Для обычного дня (не турбо) с типом выполнения `.sets`, `plannedCount = 6` и глобальным номером подхода 7 должен возвращаться индекс упражнения 1
  - Проверка: Для обычных дней: `exerciseIndex = (number - 1) / plannedCount`
- Тест: Для турбо-дня с кругами (день 92) логика не должна изменяться
  - Проверка: Для турбо-дней с кругами используется та же логика, что и для обычных дней с кругами

**Шаг 4: Написать тесты для `WorkoutViewModel.getNavigationTitle()` для турбо-дней с подходами**
- Тест: Для дня 93 заголовок навигации для подхода должен показывать правильный номер подхода для упражнения
  - Проверка: Для первого подхода первого упражнения заголовок должен быть "Подход 1 из 1"
  - Проверка: Для первого подхода второго упражнения заголовок должен быть "Подход 1 из 1"
- Тест: Для обычных дней (не турбо) заголовок навигации должен работать как раньше
  - Проверка: Для первого подхода первого упражнения заголовок должен быть "Подход 1 из 6" (если `plannedCount = 6`)
  - Проверка: Для второго подхода первого упражнения заголовок должен быть "Подход 2 из 6"

**Шаг 5: Реализовать исправление в `WorkoutViewModel.initializeStepStates()`**
- Заменить глобальный счетчик `var setNumber = 1` на локальную переменную цикла `for setNumber in 1 ... setsPerExercise`
- Изменить логику создания подходов, чтобы она соответствовала логике в основном приложении:
  ```swift
  for _ in trainings {
      for setNumber in 1 ... setsPerExercise {
          stepStates.append(
...
- Убедиться, что метод `getExerciseSteps()` корректно работает с новой логикой создания подходов
- Запустить тесты из Шага 2 - они должны пройти

**Шаг 7: Реализовать метод определения текущего упражнения в `WorkoutViewModel`**
- Добавить новый метод `getCurrentExerciseIndex(for globalSetNumber: Int) -> Int?` в `WorkoutViewModel`
- Логика метода:
  - Для турбо-дней с подходами (дни 93, 95, 98): `exerciseIndex = globalSetNumber - 1` (так как по одному подходу на упражнение)
  - Для обычных дней (не турбо) с подходами: `exerciseIndex = (globalSetNumber - 1) / plannedCount` (сохранить текущую логику)
  - Для кругов (включая турбо-дни с кругами): возвращать `nil` (так как для кругов показываются все упражнения)
- **Важно:** Метод должен корректно определять тип выполнения через `getEffectiveExecutionType()` и проверять `isTurboWithSets()` для правильной логики
- Запустить тесты из Шага 3 - они должны пройти

**Шаг 8: Обновить `WorkoutView.makeExerciseView()` для использования нового метода**
- Заменить логику определения `exerciseIndex` в `WorkoutView.makeExerciseView()` (строки 128-133) на вызов нового метода из `WorkoutViewModel`
- **Важно:** Сохранить существующую логику для обычных дней (не турбо) - использовать новый метод, который вернет тот же результат
- **Важно:** Сохранить существующую логику для турбо-дней с кругами - для кругов показываются все упражнения (как сейчас)
- **Важно:** Для турбо-дней с подходами теперь должно показываться правильное упражнение для каждого подхода (меняется упражнение при переходе к следующему подходу)
- Добавить тесты для проверки, что логика для обычных дней и турбо-дней с кругами не изменилась
