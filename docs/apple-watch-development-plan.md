...
Настройка проекта, Codable модели, сервисы, ViewModels, интеграция с iPhone, UI экраны, локализация.
...
- ✅ Тест `testMakeScreenshots()` создан. ⏳ Требуется тестирование генерации скриншотов через fastlane (Шаг 6)
...
##### 8.4.1-8.4.2 Анализ логов и исправление багов ✅
...
##### 8.6.1-8.6.2 Оптимизация синхронизации и рефакторинг ✅
...
Компиляция для watchOS, безопасное извлечение опционалов, логирование на русском, локализация. ⏳ Требуется тестирование на реальных устройствах.

#### 8.8-8.9 Интеграционные тесты и проверка UI компонентов ✅

#### 8.10 Финальная проверка и рефакторинг ⚠️ **ЧАСТИЧНО ВЫПОЛНЕНО**
✅ Все тесты проходят, форматирование проверено, основные проблемы исправлены. ⏳ Требуется тестирование на реальных устройствах, оптимизация размера/производительности/энергопотребления.

### Этап 10: Синхронизация времени отдыха между iPhone и часами ✅ **ВЫПОЛНЕНО**

**Цель:** Передавать длительность отдыха (`restTime`) из основного приложения в часы через WatchConnectivity, чтобы на часах использовалось то же значение, что настроено в iPhone приложении.

**Текущее состояние:**
- ✅ `restTime` сохраняется в `UserDefaults` в `AppSettings` (ключ `Constants.restTimeKey = "WorkoutTimer"`)
- ✅ На часах используется значение из `connectivityService.restTime`, если оно доступно, иначе дефолтное значение `Constants.defaultRestTime`
- ✅ `restTime` синхронизируется между iPhone и часами через `WatchStatusMessage` и `WatchConnectivityService`

**Важно:** ✅ После реализации синхронизации, при изменении `restTime` на экране `MoreScreen` данные автоматически отправляются на часы через `StatusManager` (при следующем вызове `sendCurrentStatus` или `updateApplicationContextOnWatch`). Специальная отправка при изменении `restTime` не требуется - синхронизация происходит автоматически в рамках существующей логики `StatusManager`.

**Реализация (TDD подход):**

#### 10.1-10.2 WatchStatusMessage с restTime ✅
- [x] Добавлены тесты для передачи `restTime` в `message` и `applicationContext`
- [x] Добавлено опциональное свойство `restTime: Int?` в `WatchStatusMessage`
- [x] Обновлены вычисляемые свойства для включения `restTime` в словари

#### 10.3-10.4 StatusManager для чтения и передачи restTime ✅
- [x] Добавлены тесты для чтения `restTime` из `UserDefaults`
- [x] Добавлено вычисляемое свойство `restTimeFromUserDefaults` в `StatusManager`
- [x] Обновлены методы `updateApplicationContextOnWatch` и `sendCurrentStatus` для передачи `restTime`

#### 10.5-10.6 WatchConnectivityService для получения и хранения restTime ✅
- [x] Добавлены тесты для извлечения `restTime` из `applicationContext` и команды `authStatus`
- [x] Добавлено свойство `restTime: Int?` в протокол и сервис
- [x] Обновлены методы `handleApplicationContext` и `handleReceivedMessage` для сохранения `restTime`

#### 10.7-10.8 WorkoutPreviewViewModel для использования restTime ✅
- [x] Добавлены тесты для использования `restTime` из `connectivityService`
- [x] Обновлен метод `loadData(day:)` для получения `restTime` из `connectivityService.restTime` с fallback на `Constants.defaultRestTime`
...
- [x] Добавлены интеграционные тесты для полного цикла синхронизации
- [x] Проверена автоматическая синхронизация при изменении `restTime` на экране `MoreScreen`
- [x] Проверено использование дефолтного значения при отсутствии `restTime`
- [x] Код отформатирован, проект компилируется без ошибок

**Статус:** ✅ **ВЫПОЛНЕНО** (реализовано через TDD подход)

**Приоритет:** СРЕДНИЙ - улучшение UX, синхронизация настроек между устройствами.

**Связанные файлы:**
- `SwiftUI-SotkaApp/Services/WatchStatusMessage.swift` - структура для передачи данных
- `SwiftUI-SotkaApp/Services/StatusManager.swift` - чтение `restTime` из `UserDefaults` и отправка на часы
- `SotkaWatch Watch App/Services/WatchConnectivityService.swift` - получение и хранение `restTime`
- `SotkaWatch Watch App/Services/WatchConnectivityServiceProtocol.swift` - протокол с свойством `restTime`
- `SotkaWatch Watch App/ViewModels/WorkoutPreviewViewModel.swift` - использование `restTime` из `connectivityService`
- `SwiftUI-SotkaApp/Extensions/RestTimeEnvironmentKey.swift` - опциональное использование Environment

### Этап 9: Известные баги и проблемы

#### 9.1 Проблема с синхронизацией currentDay при запуске приложения на часах ✅ **ИСПРАВЛЕНО**

**Проблема:** Application Context отправлялся без `currentDay` при активации WCSession, когда `didLoadInitialData = false`, что приводило к отображению неправильного дня.

**Решение:** Исправлена логика `sendApplicationContextOnActivation()` - Application Context отправляется только после завершения синхронизации с полными данными. Добавлены тесты (58 тестов проходят).

**Статус:** ✅ **ИСПРАВЛЕНО** (25.12.2025)

#### 9.2 Неправильная логика создания подходов для турбо-дней с подходами на часах ✅ **ИСПРАВЛЕНО**

**Проблема:** Для турбо-дней с подходами (дни 93, 95, 98) создавалось неправильное количество подходов из-за использования глобального счетчика вместо локальной переменной цикла.

**Решение:** Исправлена логика создания подходов, `getNavigationTitle()` и определение упражнения. Добавлены тесты (176 тестов проходят).

**Статус:** ✅ **ИСПРАВЛЕНО** (24.12.2025)
...
- `WatchStatusMessage` - структура для преобразования данных статуса в сообщение для отправки на часы (isAuthorized, currentDay, currentActivity, restTime)
...
- `authStatus` - передает статус авторизации, текущий день, текущую активность и время отдыха (используется через `sendMessage`)
...
- `restTime: Int?` - опциональное поле (время отдыха между подходами/кругами в секундах, читается из UserDefaults на iPhone)
...
- **Итеративный процесс**: Тест → Код → `make format` → Настройка defaults через MCP → Запуск тестов через MCP (`test_sim()`) или через терминал с `required_permissions: ['all']` → Следующий тест
...
### Рефакторинг архитектуры WatchConnectivity ✅
Рефакторинг выполнен с соблюдением TDD, все тесты проходят.
...
#### Шаг 1-5: Настройка fastlane для Watch скриншотов ✅
Устройства выбраны, создан `Snapfile.watch`, тест `testMakeScreenshots()` для 5 экранов, lane `watch_screenshots`, настроена мок-сессия.
...
#### Шаг 7: Интеграция в Makefile ✅
Команда `make watch_screenshots` добавлена. ⏳ Требуется тестирование.
