...
**Выполненные баги (1-6, 7-10, 11-12):** ✅
- Авторизация, синхронизация активностей, сохранение тренировок, отображение количества кругов/подходов, обновление локальных данных после сохранения
...
**Проблема (решена):** 
Множественные синхронизации при запуске приложения приводили к отправке нескольких одинаковых сообщений `PHONE_COMMAND_AUTH_STATUS` и избыточным обновлениям статуса авторизации на часах.

**Статус:**
- ✅ **Этапы 1-7, 9, 8.1-8.3 выполнены** - все основные проблемы решены
- ⏳ **Этап 8.4 в ожидании** - тестирование на реальных устройствах

**Проблемы, обнаруженные при тестировании на реальных устройствах:**

1. ✅ **Дублирование вызовов `loadData()`** - решено (Этап 8.1)
2. ⚠️ **Избыточное логирование "Проверка статуса авторизации"** - некритично, исправление не требуется
3. ✅ **Двойная проверка "Application context data is nil"** - решено (Этап 8.2)
4. ✅ **Множественные логи "Активность дня X не установлена"** - решено (Этап 8.2)
5. ✅ **Отсутствие `currentDay` в Application Context при активации WCSession** - исправлено (Этап 8.3)

**План исправления новых проблем (дополнительные этапы):**

### Этап 8.1: Предотвратить дублирование вызовов `loadData()` ✅ **ВЫПОЛНЕНО**
**Проблема:** `loadData()` вызывался дважды при запуске (из `.task` и `updateCurrentDayFromConnectivity()`)

**Решение:** Добавлена проверка `isLoading` в начале `loadData()` для предотвращения параллельных вызовов. Тест `preventsConcurrentLoadDataCalls()` проходит.

### Этап 8.2: Оптимизировать логирование ✅ **ВЫПОЛНЕНО**
**Проблема:** Избыточное логирование засоряло логи

**Решение:** Добавлена дедупликация для "Активность дня X не установлена" через `lastLoggedActivityState`. Удален избыточный лог "Application context data is nil".

### Этап 8.3: Обеспечить передачу `currentDay` в Application Context при активации WCSession ✅ **ВЫПОЛНЕНО**
**Проблема (решена):** При активации WCSession `sendApplicationContextOnActivation()` вызывался до инициализации `currentDayCalculator`, поэтому Application Context отправлялся только с `isAuthorized`, без `currentDay`.

**Решение:** Добавлена проверка `didLoadInitialData` в `sendApplicationContextOnActivation()`. Если данные не загружены, отправляется только `isAuthorized`. После загрузки `sendCurrentStatus()` обновляет Application Context с `currentDay`. Тесты проходят.

### Этап 8.4: Тестирование на реальных устройствах ⏳ **В ОЖИДАНИИ**
- Протестировать на реальных устройствах после исправлений
- Убедиться, что дублирование вызовов устранено
- Проверить, что логи стали чище и понятнее
- Убедиться, что `currentDay` передается в Application Context после загрузки данных
- Проверить, что часы могут работать без необходимости открывать приложение на iPhone сразу после запуска

**Приоритет:**
- **СРЕДНИЙ:** Критическая проблема решена (Этап 8.3), требуется проверка на реальных устройствах
- Тесты unit-тесты проходят, требуется валидация на реальных устройствах


**Пошаговый план исправления в формате TDD:**

### Этап 1: Убрать дублирующий вызов `sendCurrentStatus()` из `getStatus()` ✅ **ВЫПОЛНЕНО**
Убран дублирующий вызов до синхронизации. Тест `testGetStatusSendsStatusOnlyOnceAfterSync()` проходит.

### Этап 2: Добавить дедупликацию в `sendCurrentStatus()` ✅ **ВЫПОЛНЕНО**
Добавлены `lastSentStatus` и `hasStatusChanged()` в `StatusManager`. Дедупликация для `sendMessage`. Тест проходит.

### Этап 3: Разделить логику отправки `applicationContext` и `sendMessage` ✅ **ВЫПОЛНЕНО**
Логика разделена: `applicationContext` всегда, `sendMessage` только при `isReachable` и изменении данных.

### Этап 4: Добавить дедупликацию на часах в `handleReceivedMessage()` ✅ **ВЫПОЛНЕНО**
Добавлены `lastProcessedStatus` и `hasStatusChanged()` в `WatchConnectivityService`. Дедупликация только для `isAuthorized`. Тест проходит.

### Этап 5: Добавить дедупликацию на часах в `handleApplicationContext()` ✅ **ВЫПОЛНЕНО**
Используется `lastProcessedStatus` из этапа 4. Дедупликация только для `isAuthorized`. Тест проходит.

### Этап 6: Предотвратить лишние синхронизации при запуске ✅ **ВЫПОЛНЕНО**
Добавлена проверка `didLoadInitialData` в `sendDayDataToWatch()`.

### Этап 7: Оптимизировать `sendApplicationContextOnActivation()` ✅ **ВЫПОЛНЕНО**
Используется `lastSentStatus` и `hasStatusChanged()` для предотвращения дублирования.

### Этап 8: Интеграционные тесты для проверки полного сценария запуска ⏸️ **ОТЛОЖЕНО**

**Цель:** Убедиться, что при полном сценарии запуска приложения синхронизация происходит оптимально

**Статус:** ⏸️ Отложено
- Основная функциональность проверена на реальных устройствах и работает корректно
- Интеграционные тесты могут быть добавлены позже для более детальной проверки
- Приоритет: низкий (основные проблемы решены)

### Этап 9: Проверка вызовов из UI компонентов ✅ **ВЫПОЛНЕНО**
Все вызовы происходят только при действиях пользователя.

### Этап 10: Финальная проверка и рефакторинг ⚠️ **ЧАСТИЧНО ВЫПОЛНЕНО**
**Статус:** ⚠️ Частично выполнено
- ✅ Все тесты проходят, форматирование проверено, основные проблемы исправлены (Этапы 8.1-8.3)
- ⏳ Требуется тестирование на реальных устройствах (см. Этап 8.4)

- [ ] Оптимизация размера приложения
- [ ] Оптимизация производительности
- [ ] Оптимизация энергопотребления

##### 8.6.2 Рефакторинг метода `loadData()` в `HomeViewModel` ⏳ **ЗАПЛАНИРОВАНО**

**Проблема:**
- Метод `loadData()` в `HomeViewModel` слишком большой (~94 строки) и сложный для понимания
- Содержит множественную вложенную логику: проверка авторизации, получение `currentDay`, обработка `currentActivity`, загрузка данных тренировки
- Сложно тестировать и поддерживать из-за большого количества ответственностей в одном методе

**Цель рефакторинга:**
- Разбить метод на более мелкие, специализированные методы
- Улучшить читаемость и поддерживаемость кода
- Упростить тестирование отдельных частей логики
- Соблюдать принцип единственной ответственности (Single Responsibility Principle)

**План рефакторинга:**
1. Выделить метод для проверки авторизации и получения `currentDay`
2. Выделить метод для обработки `currentActivity` из `applicationContext`
3. Выделить метод для запроса `currentActivity` с iPhone
4. Выделить метод для загрузки данных тренировки
5. Упростить основной метод `loadData()`, оставив только координацию вызовов

**Приоритет:**
- **НИЗКИЙ:** Запланировано после завершения тестирования на реальных устройствах (Этап 8.4)
- Рефакторинг не влияет на функциональность, но улучшит качество кода

#### 8.7 Финальная проверка

##### 8.7.1 Проверка синхронизации ✅

**Результаты:** ✅ Синхронизация активностей работает корректно, проблемы множественных синхронизаций при запуске решены (см. раздел 8.6.1, Этапы 8.1-8.3)
