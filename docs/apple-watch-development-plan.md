...
Предотвращено дублирование вызовов `loadData()`, оптимизировано логирование, обеспечена передача `currentDay` в Application Context, реализована дедупликация в `sendCurrentStatus()`. ⏳ Требуется тестирование на реальных устройствах (Этап 8.4.3).

##### 8.6.2 Рефакторинг метода `loadData()` в `HomeViewModel` ✅
Метод разбит на специализированные методы. Все тесты проходят (164 теста).
...
#### 8.8 Интеграционные тесты для проверки полного сценария запуска ✅
Реализованы тесты полных сценариев запуска, дедупликации, обработки ошибок. Все тесты проходят успешно.

#### 8.9 Проверка вызовов из UI компонентов ✅
...
**Важно:** ✅ Синхронизация через `sendMessageToWatch` и `updateApplicationContext`
...
#### Шаг 1: Проверка доступных Watch симуляторов

**Цель:** Определить точные имена доступных Watch симуляторов для использования в конфигурации.

**Действия:**
- [ ] Запустить команду `xcrun simctl list devices available | grep -i "watch"` для получения списка доступных Watch симуляторов
- [ ] Выбрать модели для скриншотов (рекомендуется: разные размеры экранов, например, 42mm и 46mm)
- [ ] Записать точные имена устройств для использования в Snapfile
...
#### Шаг 2: Настройка UI-тестов для Watch приложения
...
- Создать lane, который использует этот файл:
  ...
  lane :watch_screenshots do
    capture_screenshots(
      scheme: "SotkaWatch Watch AppUITests",
      snapshot_file_path: "./fastlane/Snapfile.watch"
    )
  end
  ```
...
**Цель:** Подготовить тестовые данные для корректного отображения экранов на скриншотах.

**Действия:**
- [ ] Определить, какие данные нужны для каждого экрана:
  - [ ] Авторизованный пользователь
  - [ ] Текущий день программы
  - [ ] Выбранная активность (если нужно)
  - [ ] Данные тренировки (если нужно)
- [ ] Настроить моки или тестовые данные в UI-тестах
- [ ] Убедиться, что данные локализованы для разных языков

**Примечания:**
- Можно использовать launch arguments для передачи тестовых данных в приложение
- Или настроить моки в UI-тестах для симуляции нужных состояний
...
#### Шаг 7: Интеграция в Makefile (опционально)
