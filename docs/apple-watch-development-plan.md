...
9. **Изменение количества подходов/кругов через WorkoutStepperView не сохраняется на экране WorkoutPreviewView (часы)** ✅ Исправлено
   - **Решение:** Изменена логика `displayedCount` в `WorkoutPreviewViewModel` для часов: теперь используется `plannedCount ?? count` вместо `count ?? plannedCount`
   - **Логика:** Приоритет у `plannedCount` - если он установлен, отображается он, иначе используется `count`
   - **Результат:** При редактировании тренировки изменения `plannedCount` теперь корректно отображаются на экране `WorkoutPreviewView`
   - **Тесты:** Обновлены unit-тесты для соответствия новой логике

10. **Изменение количества кругов/подходов не работает в iPhone приложении при редактировании тренировки** ✅ Исправлено
   - **Решение:** Изменена логика `displayedCount` в `WorkoutPreviewViewModel` для iPhone: теперь используется `plannedCount ?? count` вместо `count ?? plannedCount`
   - **Логика:** Приоритет у `plannedCount` - если он установлен, отображается он, иначе используется `count`
   - **Результат:** При редактировании тренировки изменения `plannedCount` теперь корректно отображаются на экране `WorkoutPreviewScreen`
   - **Тесты:** Обновлены unit-тесты для соответствия новой логике
