...
**Выполненные баги (1-6, 7-10, 12):** ✅
- Авторизация, синхронизация активностей, сохранение тренировок, отображение количества кругов/подходов
...
8. **При сохранении тренировки на iPhone данные не отправляются на часы или отправляются не полностью** ✅ Исправлено
   - Добавлена обработка `PHONE_COMMAND_SEND_WORKOUT_DATA` и обновление локальных данных в `HomeViewModel`

9-10. **Изменение количества кругов/подходов не работает при редактировании тренировки** ✅ Исправлено

12. **WorkoutPreviewScreen показывает неправильное количество кругов/подходов при повторном открытии** ✅ Исправлено
   - **Проблема:** При редактировании сохраненной тренировки отображался `plannedCount` вместо `count`/`executionCount`
   - **Решение:** Исправлена логика `displayedCount` (`count ?? plannedCount`), обновлены методы `updatePlannedCount` для работы с сохраненными тренировками

11. **После сохранения тренировки на часах локальные данные не обновляются** ⚠️ Требует исправления
   - **Проблема:** После сохранения измененной тренировки на часах данные успешно отправляются на iPhone, но на часах локальные данные не обновляются - остаются старые данные
   - **Ожидаемое поведение:** После успешного сохранения локальные данные должны обновляться актуальными значениями
   - **План исправления (TDD):**
     1. Обновить локальные данные в `WorkoutPreviewViewModel` после успешного сохранения
     2. Обновить данные в `HomeViewModel` после получения `PHONE_COMMAND_AUTH_STATUS` с обновленным `currentActivity`
     3. Оптимизировать отправку данных на часы после сохранения (использовать данные из ViewModel)
     4. Вынести логику декодирования данных команды `saveWorkout` в статический метод расширения `WatchStatusMessage`
     5. Написать тесты
...
**Результаты:** ✅ Синхронизация активностей работает корректно, выявлена избыточность логов (см. раздел 8.6.1)
