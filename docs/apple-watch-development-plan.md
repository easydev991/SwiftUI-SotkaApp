...
### Этап 2: Добавление Codable к моделям ✅
...
### Этап 6: UI экранов ✅
- ✅ Все экраны созданы, платформо-специфичные компоненты и UIKit API корректно обработаны
...
#### 8.1 Исправление падающих unit-тестов основного приложения ✅
- ✅ Исправлено 18+ тестов: передача `modelContainer` в `MockStatusManager.create()`

#### 8.2 Интеграционные тесты для Watch App ✅
- ✅ 18 интеграционных тестов: синхронизация (5), обработка команд (6), обработка ошибок (7)
...
#### 8.4.1 Анализ логов синхронизации (результаты ручного тестирования)

**Анализ логов при запуске приложений:**

1. **При запуске часов:**
   - ✅ `applicationContext` успешно получен при активации WCSession
   - ✅ Все данные обработаны корректно: `isAuthorized`, `currentDay`, `currentActivity`
   - ⚠️ Логируется "Application context data is nil" дважды (избыточность)
   - ✅ `loadData()` использует `currentActivity` из `applicationContext` без запроса к iPhone

2. **При открытии iPhone приложения:**
   - ⚠️ Отправляется несколько `applicationContext` и `sendMessage`:
     - Первый `applicationContext` содержит только `isAuthorized`
     - Второй `applicationContext` содержит полные данные (`isAuthorized`, `currentDay`, `currentActivity`)
     - Затем отправляются несколько `sendMessage` с PHONE_COMMAND_AUTH_STATUS (дублирование)
   - ⚠️ На часах обрабатываются все сообщения, что приводит к избыточным логам

**Анализ логов при изменении активности:**

1. **При изменении активности на iPhone:**
   - ✅ `applicationContext` отправляется с новым `currentActivity`
   - ✅ На часах `currentActivity` обновляется корректно
   - ⚠️ После `applicationContext` отправляется `sendMessage` с PHONE_COMMAND_AUTH_STATUS (избыточность)
   - ⚠️ На часах обрабатываются оба сообщения (дублирование обработки)

2. **При изменении активности на часах:**
   - ✅ Команда отправляется на iPhone
   - ✅ iPhone обрабатывает команду и отправляет `applicationContext` обратно на часы
   - ⚠️ После `applicationContext` отправляется `sendMessage` с PHONE_COMMAND_AUTH_STATUS (избыточность)
   - ⚠️ На часах обрабатываются оба сообщения (дублирование обработки)

3. **При удалении активности:**
   - ✅ `applicationContext` отправляется без `currentActivity` (корректно обрабатывается как удаление)
   - ✅ На часах `currentActivity` становится `nil` корректно
   - ⚠️ После `applicationContext` отправляется `sendMessage` с PHONE_COMMAND_AUTH_STATUS (избыточность)

**Анализ новых логов (дополнительное тестирование):**

4. **При запуске часов:**
   - ⚠️ Логируется "Application context data is nil" дважды (подтверждение избыточности)
   - ✅ `applicationContext` получен при активации WCSession с данными `currentDay` и `isAuthorized`
   - ✅ `currentActivity` удаляется из `applicationContext` (активность не установлена)
   - ⚠️ Множественные запросы текущей активности дня 15 с iPhone (4 раза подряд)
   - ⚠️ Множественные проверки статуса авторизации (3 раза подряд)

5. **При запуске iPhone приложения:**
   - ⚠️ Отправляется несколько `applicationContext` и `sendMessage`:
     - `applicationContext` с `isAuthorized` и `currentDay`
     - Затем 3 `sendMessage` с `PHONE_COMMAND_AUTH_STATUS` (дублирование)
   - ⚠️ На часах обрабатываются все сообщения, что приводит к избыточным логам

**Выводы:**

✅ **Функциональность работает корректно:**
- Все сценарии синхронизации работают правильно
- Данные синхронизируются между iPhone и часами
- Визуально все отображается корректно

⚠️ **Избыточность логов и событий (подтверждено дополнительным тестированием):**
- `sendCurrentStatus()` отправляет и `applicationContext`, и `sendMessage`
- Это приводит к двойной обработке на часах
- Множественные сообщения при каждом изменении активности
- Дублирование проверки "Application context data is nil"
- Множественные запросы данных с iPhone (4+ раза подряд)
- Множественные проверки статуса авторизации (3+ раза подряд)

**Рекомендации:**
- См. раздел 8.6.1 "Оптимизация синхронизации между iPhone и часами"
- Оптимизация не критична для работы приложения, но желательна для чистоты кода и производительности
- См. также раздел 8.4.3 "План исправления багов синхронизации тренировок" для критичных багов
...
1. **Кнопка "Проверить авторизацию" бесполезна, если приложение на iPhone закрыто** ✅
   - ✅ Реализован `updateApplicationContext` для отправки статуса с iPhone
   - ✅ `applicationContext` отправляется при активации WCSession и обрабатывается на часах

2. **ApplicationContext не обрабатывается полностью на часах при запуске** ✅
   - ✅ Обновлен `handleApplicationContext()` для обработки всех данных
   - ✅ Добавлена проверка `receivedApplicationContext` при активации WCSession
...
4. **При выборе активности в основном приложении ничего не происходит в часах** ⚠️ Частично решено
   - ✅ Добавлен вызов `sendCurrentStatus` в `HomeActivitySectionView.actionFor()`
   - ⚠️ Удаление и изменение активности обрабатываются в баге 4

5. **При удалении или изменении активности в основном приложении активность на часах не обновляется** ✅ Выполнено
   - ✅ Отправка `applicationContext` с `currentActivity` реализована
   - ✅ Обработка `currentActivity` на часах реализована

6. **currentActivity из applicationContext не обрабатывается на часах** ✅
   - ✅ Добавлено свойство `currentActivity` в `WatchConnectivityService` с callback
   - ✅ Обновлен `HomeViewModel` для использования `currentActivity` из `applicationContext`
   - ✅ Добавлены тесты

**Новые баги, найденные при тестировании:**

7. **При сохранении тренировки на часах как "пройденной" экран не закрывается и не все данные отправляются на iPhone** ⚠️ Требует исправления
   - **Проблема:** При нажатии "Сохранить как пройденную" на часах:
     - Экран `WorkoutPreviewView` не закрывается автоматически
     - В iPhone приходят не все данные тренировки (только `count`, но не `duration`, `executionType`, `comment`)
   - **Логи с часов:** "Отправка результата тренировки на iPhone: день 15, количество 4"
   - **Логи с iPhone:** После сохранения приходят только `applicationContext` и `sendMessage` с `PHONE_COMMAND_AUTH_STATUS`, но не все данные тренировки
   - **Ожидаемое поведение:**
     - Экран должен закрываться после успешного сохранения
     - Все данные тренировки должны отправляться на iPhone (`count`, `duration`, `executionType`, `comment`)
     - iPhone должен обрабатывать сохранение тренировки и отправлять подтверждение на часы

8. **При сохранении тренировки на iPhone данные не отправляются на часы** ⚠️ Требует исправления
   - **Проблема:** При сохранении тренировки на iPhone (как "пройденной" или после прохождения всей тренировки):
     - Данные не отправляются на часы через WatchConnectivity
     - Часы не обновляют отображение активности дня
     - Логов отправки данных на часы нет
   - **Сценарии:**
     - Открытие `WorkoutPreviewScreen` на iPhone → нажатие "Сохранить как пройденную" → ничего не отправляется на часы
     - Прохождение всей тренировки на iPhone → нажатие "Сохранить" → ничего не отправляется на часы
   - **Ожидаемое поведение:**
     - После сохранения тренировки на iPhone должен отправляться `applicationContext` с обновленным `currentActivity`
     - Часы должны получать обновление и отображать тренировку как активность дня
     - Должна быть синхронизация данных тренировки между iPhone и часами
...
**Баг 7: Сохранение тренировки на часах не закрывает экран и не отправляет все данные**

**Анализ проблемы:**
1. При сохранении тренировки на часах отправляется только `count`, но не `duration`, `executionType`, `comment`
2. Экран `WorkoutPreviewView` не закрывается после успешного сохранения
3. Нужно проверить метод `saveWorkoutAsCompleted()` в `WorkoutPreviewViewModel` на часах

**План исправления:**
1. **Проверить отправку данных тренировки:**
   - Убедиться, что в `WatchWorkoutService.saveWorkout()` отправляются все поля: `count`, `duration`, `executionType`, `comment`
   - Проверить формат команды `WATCH_COMMAND_SAVE_WORKOUT` - все поля должны быть включены
   - Добавить логирование всех отправляемых данных для отладки

2. **Исправить закрытие экрана:**
   - После успешного сохранения тренировки на часах должен вызываться `dismiss()` или обновление навигации
   - Проверить обработку успешного ответа от iPhone в `WorkoutPreviewViewModel`
   - Добавить закрытие экрана в callback успешного сохранения

3. **Проверить обработку на iPhone:**
   - Убедиться, что `StatusManager.handleWatchCommand()` для `WATCH_COMMAND_SAVE_WORKOUT` обрабатывает все поля
   - Проверить сохранение всех данных в SwiftData через `DailyActivitiesService`
   - Добавить отправку подтверждения на часы после успешного сохранения

4. **Тестирование:**
   - Написать unit-тесты для проверки отправки всех полей тренировки
   - Написать интеграционные тесты для проверки полного цикла сохранения
   - Протестировать на реальных устройствах

**Баг 8: Сохранение тренировки на iPhone не отправляет данные на часы**

**Анализ проблемы:**
1. При сохранении тренировки на iPhone не вызывается отправка данных на часы
2. Нужно найти места сохранения тренировки в основном приложении и добавить отправку `applicationContext`
3. Проверить `WorkoutPreviewScreen` и `WorkoutScreen` на iPhone

**План исправления:**
1. **Найти места сохранения тренировки:**
   - Проверить `WorkoutPreviewScreen` - метод сохранения "как пройденной"
   - Проверить `WorkoutScreen` - метод сохранения после прохождения тренировки
   - Проверить `DailyActivitiesService` - метод сохранения активности типа `workout`

2. **Добавить отправку данных на часы:**
   - После успешного сохранения тренировки вызывать `sendCurrentStatus()` или `updateApplicationContextOnWatch()`
   - Убедиться, что `applicationContext` содержит обновленный `currentActivity` с типом `workout`
...
1. **Исправить баг 7 (сохранение на часах):**
   - Проверить и исправить отправку всех данных тренировки
   - Добавить закрытие экрана после успешного сохранения
   - Написать тесты

2. **Исправить баг 8 (сохранение на iPhone):**
   - Найти все места сохранения тренировки
   - Добавить отправку `applicationContext` после сохранения
   - Написать тесты

3. **Интеграционное тестирование:**
   - Протестировать полный цикл: сохранение на часах → синхронизация с iPhone
   - Протестировать полный цикл: сохранение на iPhone → синхронизация с часами
   - Проверить все сценарии сохранения тренировки
...
- `SotkaWatch Watch App/ViewModels/WorkoutPreviewViewModel.swift` - сохранение на часах
- `SotkaWatch Watch App/Services/WatchWorkoutService.swift` - отправка данных с часов
- `SwiftUI-SotkaApp/Services/StatusManager.swift` - обработка команды сохранения на iPhone
- `SwiftUI-SotkaApp/Screens/Workout/WorkoutPreviewScreen.swift` - сохранение на iPhone
- `SwiftUI-SotkaApp/Screens/Workout/WorkoutScreen.swift` - сохранение после прохождения
- `SwiftUI-SotkaApp/Services/DailyActivitiesService.swift` - сохранение активности
...
**Проблемы, выявленные при анализе логов:**

1. **Дублирование отправки данных:**
   - `sendCurrentStatus()` отправляет и `applicationContext`, и `sendMessage` (PHONE_COMMAND_AUTH_STATUS)
   - Это приводит к двойной обработке на часах: сначала через `applicationContext`, потом через `sendMessage`
   - На часах обрабатываются оба источника данных, что создает избыточные логи и обработку

2. **Множественные сообщения при изменении активности:**
   - При изменении активности отправляется:
     - `applicationContext` (через `updateApplicationContextOnWatch`)
     - `sendMessage` с PHONE_COMMAND_AUTH_STATUS (через `sendMessageToWatch`)
   - На часах приходят оба сообщения, что приводит к дублированию обработки
   - Пример из логов: при изменении активности на iPhone приходит `applicationContext`, затем `sendMessage`, затем еще один `sendMessage`

3. **Избыточные сообщения при запуске:**
   - При открытии iPhone приложения отправляется несколько `applicationContext` и `sendMessage`
   - Первый `applicationContext` содержит только `isAuthorized`
   - Второй `applicationContext` содержит полные данные
   - Затем отправляются несколько `sendMessage` с одинаковыми данными

4. **Двойная проверка applicationContext на часах:**
   - Логируется "Application context data is nil" дважды при запуске часов
   - Это может быть из-за проверки до активации WCSession и после активации

**Анализ текущей реализации:**

- `sendCurrentStatus()` всегда отправляет:
  1. `applicationContext` (работает даже когда приложение закрыто)
  2. `sendMessage` с PHONE_COMMAND_AUTH_STATUS (требует активного соединения)

- На часах обрабатываются:
  1. `applicationContext` через `handleApplicationContext()` → обновляет `currentDay`, `currentActivity`, `isAuthorized`
  2. `sendMessage` через `handleReceivedMessage()` → обрабатывает команду PHONE_COMMAND_AUTH_STATUS → обновляет `isAuthorized` (дублирование)
...
##### 8.7.1 Проверка синхронизации ⚠️ (ручное тестирование)

**Результаты:**
- ✅ Выбор, изменение и удаление активности синхронизируются корректно между iPhone и часами через `applicationContext`
- ⚠️ Тип активности `workout` протестирован, выявлены критические баги (см. раздел 8.4.3):
  - Баг 7: Сохранение тренировки на часах не закрывает экран и не отправляет все данные
  - Баг 8: Сохранение тренировки на iPhone не отправляет данные на часы
- ⚠️ Выявлена избыточность логов и событий (см. раздел 8.6.1)
...
### Риск 7: Синхронизация данных между iPhone и часами ✅
...
✅ **Выполнено:**
- Создание протокола и структуры для отправки данных
- Перенос делегата WCSession в `StatusManager`
- Упрощение работы с WatchConnectivity (структура `WatchStatusMessage`)
- Убрана очередь запросов и force unwrap
- Рефакторинг обработки команд через Command Pattern
- Обновление тестов (см. раздел 8.1)
- Устранение зависимости от ModelContext
