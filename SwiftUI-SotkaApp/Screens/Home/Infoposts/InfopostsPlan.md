...
**Файл**: `Models/Infoposts/InfopostReadResponse.swift`
```swift
...
- Добавить методы:
  - `syncReadPosts() async throws` - синхронизация прочитанных постов с сервером
  - `markPostAsRead(day: Int?) async throws` - маркировка поста как прочитанного (принимает Int?, сам обрабатывает nil)
  - `isPostRead(day: Int) -> Bool` - проверка статуса прочитанного поста
...
.task {
    do {
        try await infopostsService.syncReadPosts()
    } catch {
...
.task {
    do {
        try await infopostsService.markPostAsRead(day: infopost.dayNumber)
    } catch {
...
- Тесты для методов синхронизации в `InfopostsService`
- Тесты для маркировки постов
