...
extension StatusManagerWatchConnectivityTests {
    ...
    struct IntegrationTests {
        ...
        @Test("Дедупликация при множественных обновлениях статуса")
        func deduplicationWithMultipleStatusUpdates() throws {
            let mockSession = MockWCSession(isReachable: true)
            let modelConfiguration = ModelConfiguration(isStoredInMemoryOnly: true)
            let modelContainer = try ModelContainer(
                for: User.self,
            ...
            statusManager.sendCurrentStatus(isAuthorized: true, currentDay: 20, currentActivity: .workout)
            ...
            statusManager.sendCurrentStatus(isAuthorized: true, currentDay: 20, currentActivity: .rest)
            #expect(mockSession.applicationContexts.count == initialApplicationContextCount + 5)
            #expect(mockSession.sentMessages.count == initialSentMessagesCount + 3)

            statusManager.sendCurrentStatus(isAuthorized: false, currentDay: nil, currentActivity: nil)
            #expect(mockSession.applicationContexts.count == initialApplicationContextCount + 6)
            #expect(mockSession.sentMessages.count == initialSentMessagesCount + 4)

            statusManager.sendCurrentStatus(isAuthorized: false, currentDay: nil, currentActivity: nil)
            #expect(mockSession.applicationContexts.count == initialApplicationContextCount + 7)
            #expect(mockSession.sentMessages.count == initialSentMessagesCount + 4)
        }
    }
}
